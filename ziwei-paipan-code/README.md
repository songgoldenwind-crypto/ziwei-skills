...
- 真太阳时校正：按出生地经度计算真太阳时，含均时差修正；同时输出校正分钟数、校正前后时辰、是否跨时辰及距最近时辰边界的分钟数。
...
- 语义层 `ai_context`：本命盘含出生四柱、十二宫索引、核心宫轴、命宫三方四正、四化落宫和空宫；运限盘另含各层命宫叠原局宫位、四化落宫与流曜落宫。
...
| `derived` | 当地时间、UTC、真太阳时、校正量、校正前后时辰、跨界／临界标记及排盘日期 |
| `四柱` | 出生年柱、月柱、日柱、时柱（按真太阳时排盘时辰） |
| `data` | 排盘原始数据；运限模式下为运限层 |
| `ai_context` | 整理后的语义层。本命盘直接给原局；运限模式分为 `原局` 与 `运限` |
